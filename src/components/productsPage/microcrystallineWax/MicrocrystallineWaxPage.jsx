...
export default function MicrocrystallineWaxPage() {
  ...
  const pageData = t('pages.microcrystalline-wax', { returnObjects: true });
  ...
  const data = {
    ...microcrystallineWaxData,
    name: pageData.name,
    description: pageData.purity,
    slider: microcrystallineWaxData.slider.map((slide, i) =>
      i === 0
        ? { ...slide, title: pageData.name, subtitle: pageData.purity }
        : slide
    ),
    commonSection: {
      ...microcrystallineWaxData.commonSection,
      title: pageData.common.title,
      description: pageData.common.description,
    },
    applicationsSection: {
      ...microcrystallineWaxData.applicationsSection,
      applications: pageData.applications.intro,
      finalProductUtilization: pageData.applications.items,
    },
    packagedResponsibly: {
      ...microcrystallineWaxData.packagedResponsibly,
      packagingOptions: (microcrystallineWaxData.packagedResponsibly?.packagingOptions || []).map(opt => ({
        ...opt,
    ...
    keyFeatures: {
      ...microcrystallineWaxData.keyFeatures,
      heading: pageData.keyFeatures.heading,
      features: (microcrystallineWaxData.keyFeatures?.features || []).map((f, i) => ({
        ...f,
