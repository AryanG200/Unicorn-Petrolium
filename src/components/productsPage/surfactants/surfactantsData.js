export const surfactantsData = {
  name: "Surfactants",
  description: "Anionic and nonionic surfactants for cleansing and emulsification",
  bannerImage: "/assets/BannerImages/productsPage/sur1.jpg",

  slider: [
    {
      title: "Surfactants",
      subtitle: "Anionic and nonionic surfactants for cleansing and emulsification",
      image: "/assets/BannerImages/productsPage/sur1.jpg",
      mobileImage: "/assets/BannerImages/productsPage/sur1.jpg"
    },
    {
      title: "",
      subtitle: "",
      image: "/assets/BannerImages/productsPage/sur2.jpg",
      mobileImage: "/assets/BannerImages/productsPage/sur2.jpg"
    }
  ],

  commonSection: {
    title: "Surfactants",
    description:
      "Surfactants find its importance in a varied range of cleaning & personal care products by reducing surface tension and enabling blending of water with dirt & other contaminants. Jointly, surfactants play a vital role in diverse formulations from helping shampoos foam up to removing grease from products. Due to its adaptable nature, it is a crucial ingredient that finds its importance in the homecare and personal care industry.",
    additionalInfo: "As surfactants are used in combination, they place themselves at the junction between two substances in a way that the hydrophilic part interacts with the water molecules while the hydrophobic part attaches with the other substance. This leads to an obstruction between the two compounds thereby decreasing surface tension and lifting the dirt from the surface for effective results.",
    brochureUrl: "#",
  },

  surfactantProducts: {
    products: [
      {
        name: "Alpha olefin sulphonate (AOS)",
        description: "Available in different grades in powder and liquid states.",
        image: "/assets/sURFACTANTS/AOS 96 (powder).jpg",
        hoverImage: "/assets/sURFACTANTS/AOS 38_ (liquid).jpg",
        pdfUrl: "/assets/PDF/Surfactants/Surfactants/Alpha%20Olefin%20Sulphonate.pdf"
      },
      {
        name: "Sodium lauryl ether sulphate (SLES) 70",
        description: "Translucent paste with no characteristic odour.",
        image: "/assets/sURFACTANTS/SLES 1.jpg",
        hoverImage: "/assets/sURFACTANTS/SLES 2.jpg",
        pdfUrl: "/assets/PDF/Surfactants/Surfactants/Sodium%20Lauryl%20Ether%20Sulphate.pdf"
      },
      {
        name: "CAPB / Betaine",
        description: "Appears colorless to light yellow liquid.",
        image: "/assets/sURFACTANTS/CAPB 1.jpg",
        hoverImage: "/assets/sURFACTANTS/CAPB 2.jpg",
        pdfUrl: "/assets/PDF/Surfactants/Surfactants/CAPB%20-%20Betaine.pdf"
      },
      {
        name: "LABSA 96%",
        description: "Appears as honey to dark brown liquid.",
        image: "/assets/sURFACTANTS/LABSA 1.jpg",
        hoverImage: "/assets/sURFACTANTS/LABSA 2.jpg",
        pdfUrl: "/assets/PDF/Surfactants/Surfactants/LABSA%2096%25.pdf"
      }
    ]
  },

  applicationsSection: {
    productName: "surfactants",
    applications:
      "Surfactants have diverse usefulness across multiple industries as shown below:",
    applicationPoints: [
      "Personal care: Enhance foaming, cleaning, and moisturising in shampoos, conditioners, body washes, soaps, and toothpaste.",
      "Cleaning agents: They improve the cleaning power and foam stability in soaps, detergents, and dishwashing liquids by removing dirt / grease.",
      "Textile industry: They also serve their importance in textile industry by improving properties like softness of the fabric, making the fabric antistatic and anti-bacterial, sweat absorbent and allowing the fabric to dry quickly in textile finishing process.",
      "Automotive industry: Due to their ability to reduce surface tension, surfactants are widely used in car cleaning agents and in engine degreases to remove dirt / grease & oil deposits.",
      "Paints & Coatings: These agents improve dispersion in automotive paints, enhance coating adhesion and uniformity, and prevent defects like streaks and bubbles during printing. They function as emulsifiers, wetting agents, and dispersants."
    ],
    finalProductUtilization: [
      "Shampoos",
      "Body Wash",
      "Face Wash",
      "Liquid Soaps",
      "Detergents",
    ],
  },

  packagedResponsibly: {
    packagingOptions: [
      { icon: "/assets/Icons/ICON/Icon 4.jpg.jpeg", type: "Bags", weight: "" },
      { icon: "/assets/Icons/icon 64 x 64 jpg-2.jpg.jpeg", type: "Flexi bags", weight: "" },
      { icon: "/assets/Icons/ICON/Icon 3.jpg.jpeg", type: "IBC containers", weight: "" },
      { icon: "/assets/Icons/ICON/Icon 1.jpg.jpeg", type: "HDPE drums", weight: "" },
    ],
  },

  relatedProducts: {
    relatedProducts: [
      { id: "emulsifying-wax", name: "Emulsifying Wax", image: "/assets/RELATED PROD IMG/NEW IMAGES/emulsifyingwax.jpg" },
      { id: "d-panthenol", name: "D-Panthenol", image: "/assets/RELATED PROD IMG/NEW IMAGES/Dpanthenol.jpg" },
      { id: "preservatives", name: "Preservatives", image: "/assets/RELATED PROD IMG/NEW IMAGES/Preservative.jpg" },
    ],
  },
};

export default surfactantsData;
